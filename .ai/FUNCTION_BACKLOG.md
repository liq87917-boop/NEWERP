...
| 7 | 采购订单 | **verified-complete（ERP-022 执行时间线；ERP-026 收货 / 结算进度只读派生；浏览器验收待 FINAL-UI-ACCEPTANCE）** | `PurchaseOrder.cs:44-86`、`PurchaseOrderController`（含 `/{id}/timeline`、`/{id}/progress`）、`OrderExecutionTimeline`（订单 + 供应商确认交期 + 入库）、`PurchaseOrderProgress`（入库派生已订/已收/未收 + 既有引用可用时的已结算/未结算，引用不唯一/不可用时金额为 null）、`order-timeline.js`、`purchase-order-progress.js`、`OrderExecutionTimelineTests`、`PurchaseOrderProgressTests`（12 用例）、`OrderTraceabilityTests`；见 `docs/订单追溯字段说明.md` §4.2 | ~~采购执行时间轴~~ → **ERP-022 已实现**；~~比价 `PurchaseQuote.IsSelected` 下游联动~~ → **ERP-020 已实现**；~~入库/结算单据进度回写（只读口径）~~ → **ERP-026 已实现派生**（不改人工字段、不写库）；仍未做：入库 / 付款单据对订单进度的自动回写、付款单按单核销明细（需结构变更任务）、采购入库成本单价从采购订单带入 | medium |
...
| 25 | 报表中心 | verified-complete（15 张 + ERP-018/ERP-029 派生报表） | `ReportController` 16 个 `[HttpGet]`；`reports.js` 15 个 `REPORTS` 键；菜单 = 原 7 张 + `ar-aging`/`container-stats`/`purchase-cost`/`tax-refund-summary`/`stock-alert`/`sales-commission`/`follow-up-due`；**ERP-018 新增 `quotation-conversion`（报价成交率）**，入口在报价单页工具栏（报表中心菜单表未改）；**ERP-029 新增 `inventory-movement`（库存移动与呆滞报表，只读派生）**，入口在库存查询页工具栏，口径见 `docs/库存单据与库存成本说明.md` §10 | 仍缺报表：佣金/代理费汇总、应付账龄、库龄分布与成本估值、生产/成本 | low-medium |
