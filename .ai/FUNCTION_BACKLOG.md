...
| 6 | 销售订单（外销合同） | **verified-complete（ERP-022 补齐执行时间线；待 Edge 门禁）** | `SalesOrder.cs:56-118`、`SalesOrderController`（含 `/{id}/timeline`）、`OrderExecutionTimeline`（订单 + 出库 + 出口单证 + 客诉，固定数量查询）、`order-timeline.js`、`OrderExecutionTimelineTests`；另见 `OrderTraceabilityTests` 与 `docs/订单追溯字段说明.md` | 订单变更申请、附件；~~订单执行跟踪时间轴~~ → **ERP-022 已实现**；SP 版单据页已退出菜单（历史 SP 表数据不迁移） | medium |
