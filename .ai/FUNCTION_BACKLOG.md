...
| 7 | 采购订单 | **verified-complete（代码 + 测试；待 Edge 门禁）** | `PurchaseOrder.cs:44-86`（归属客户/归属销售订单/代垫/供应商确认交期/税率与含税/到货进度/验货状态/合同号/结算进度）、`SchemaUpgrader.cs:891-931`、`PurchaseOrderController.cs`（关键字含合同号与归属销售订单号、`/{id}/print`、`export-excel`、税率 0~100 校验、`Calculate`/`Validate` 声明为 public 供 ERP-020 转换复用）、`modules-doc.js:150-191`、`sales-pi.js:30-37`、`bill-export.js:16-23`、`OrderTraceabilityTests`、`OrderTraceabilityUiTests` | 采购执行时间轴、~~比价 `PurchaseQuote.IsSelected` 下游联动~~（**ERP-020 已实现**：选中比价行带入预填 / 直接生成，来源留痕于备注 + 比价行 `RefOrderNo`）、入库/结算单据自动回写进度 | medium |
