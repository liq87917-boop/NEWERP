...
| 8 | 采购入库 / 销售出库 / 库存查询 | **verified-complete（ERP-023 补齐基础单位换算与取消冲回）** | `Logistics.cs`、`StockInController`/`StockOutController`、`StockUnitConversion`（包装单位 → 基础单位、批量商品查询、无效因子拒绝）、`StockUnitConversionTests`、`StockController`、`modules-doc2.js:3-29` | ~~多单位换算未参与出入库计算~~ → **ERP-023 已实现**；历史入出库仍不写 `StockMovements`；无库位/批次成本 | low-medium |
