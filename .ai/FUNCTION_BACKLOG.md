...
| 7 | 采购订单 | **verified-complete（ERP-022 补齐执行时间线；待 Edge 门禁）** | `PurchaseOrder.cs:44-86`、`PurchaseOrderController`（含 `/{id}/timeline`）、`OrderExecutionTimeline`（订单 + 供应商确认交期 + 入库）、`order-timeline.js`、`OrderExecutionTimelineTests`、`OrderTraceabilityTests` | ~~采购执行时间轴~~ → **ERP-022 已实现**；~~比价 `PurchaseQuote.IsSelected` 下游联动~~ → **ERP-020 已实现**；入库/结算单据自动回写进度仍未做 | medium |
