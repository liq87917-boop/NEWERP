...
| 5 | 供应商比价 PurchaseQuote | **verified-complete（ERP-020 补齐「选中 → 生成采购订单」；浏览器验收待 FINAL-UI-ACCEPTANCE）** | `PurchaseQuote.cs`（比价批次/多供应商/是否选中/为客户询价）、`PurchaseQuoteController`（通用 CRUD + `GET {id}/order-prefill` + `POST {id}/to-order`）、`PurchaseQuoteConversion.cs`（资格守卫 + 固定映射 + 两道重复守卫 + 服务端复核）、`modules.js`（行操作「生成采购订单」「预填采购订单」，仅 `已选中` 行显示；状态选项补 `已转采购订单`）、`purchase-order-conversion.js`（带入预填 / 直接生成 + 采购订单表单回填）、`index.html`、菜单 `purchase-quote`(`init10.sql`)、`PurchaseQuoteConversionTests`（27 用例）、`PurchaseQuoteConversionUiTests`（2 个 Edge 用例）；见 `docs/订单追溯字段说明.md` §4.1 | ① ~~`IsSelected` 选中后无「生成采购订单」下游动作~~ → **ERP-020 已实现**（带入预填 + 直接生成、来源留痕 `RefOrderNo` + 备注标记、同一比价行仅一张）；② 比价批次多行合并生成采购订单、采购价格审批、供应商回写仍未做 | low-medium |
