...
| 2 | 主数据（客户/供应商/员工/仓库/费用科目/商品/数据字典） | verified-complete（字段已补齐） | `BaseCustomer.cs:63-113`（业务性质/币种/结算方式/贸易条款/目的港/Consignee/Notify/默认唛头/账期/佣金比例/等级/信用状态/来源）；`BaseSupplier.cs:58-91`（类型/档口位置/主营/结算/开票/税率/交期/返点/微信）；`BaseOtherInfo.cs:82-143`（装箱单位/装箱数/单位换算/外箱四边/体积重/英文报关品名/退税率/品牌/认证/客户与工厂货号/MOQ/含税/安全库存上下限）；`SchemaUpgrader.cs:280-322,552-553`；字典 14 类 `modules.js:173-203`；**ERP-036 客户「指定货代」**：`BaseCustomer.ForwarderId/ForwarderName`（`[MaxLength(100)]`，`ForwarderAvailable` 为 `[NotMapped]` 读取标注）+ `BaseCustomer.cs:120-143`；`CustomerForwarderService`（写入校验 / 名称快照 / 历史引用保护 / 读取标注 / 下拉选项）+ `CustomerForwarderRules`（纯规则）；`CustomerController`（`Create`/`Update`/`GetPaged`/`GetById` override + `GET forwarder-options`）；`modules.js`（客户列表「指定货代」列 + 表单引用字段）、`bill-edit.js`（选项型引用接口 + `clearOnEmpty`）、`crud.js`（引用字段按行快照回显）；`CustomerForwarderTests`（19 例：新增 / 更换 / 清空 / 不存在 / 已删除 / 已停用 / 类型不符 / 历史引用可读与标注 / 老客户兼容 / 下拉选项 / 纯规则）；见 `docs/技术方案说明书.md` §五 基础资料 | ~~客户「指定货代」~~ → **ERP-036 已实现**（复用「其他资料」Forwarder 字典项，刻意不建外键、无生产回填；服务端校验引用并以字典项写回名称快照，客户端自由文本不被采信；字典项停用 / 删除后历史引用照常可读并显式标注「已停用/不可用」且不可再新选中；不触达订舱 / 装柜 / 报关 / 费用单据，不接外部货代系统）；商品颜色/尺码 SKU 变体、多供应商供货关系、图片资料库页面（3 图字段已有） | low |
| 3 | 询价单（CRUD/提交/审核/导出） | **verified-complete（ERP-021 补齐一键转报价；浏览器验收待 FINAL-UI-ACCEPTANCE）** | `InquiryController`、`InquiryQuotationConversion`、`modules-doc.js`、`sales-pi.js`、`InquiryQuotationConversionTests`、菜单 `inquiry-new`/`inquiry-export` | 已审核询价单支持预填 / 直接生成报价单，来源留痕、金额服务端复算、重复转换守卫；~~多轮报价版本仍按需另立任务~~ → **ERP-035 已实现报价版本链**（见 `docs/报价单与PI设计方案.md` §12） | low |
| 4 | 报价单 Quotation | **verified-complete（ERP-024 补齐 CRUD 回归；浏览器验收待 FINAL-UI-ACCEPTANCE）** | `QuotationController`（分页/详情/新增/修改与服务端合计复算、转换、打印、有效期）、`QuotationCrudRegressionTests`（创建/更新/详情/列表/无效输入 8 例）、`QuotationToPiTests`、`SalesOrderConversionTests`、`QuotationGovernanceTests`、`bill-config-ef.js`、`sales-pi.js`、`ReportService.Quotation.cs` | 转 PI、共享打印、转销售订单、有效期治理/成交率与 CRUD 回归均已覆盖；剩余功能缺口：~~报价版本号（多轮议价）~~ → **ERP-035 已实现**（版本链：根单 / 版本号 / 上一版本 + 链内版本号唯一索引 + 历史版本只读；`QuotationRevisionService` / `QuotationRevisionTests` 12 例，见 `docs/报价单与PI设计方案.md` §12） | medium |
| 5 | 供应商比价 PurchaseQuote | **verified-complete（ERP-020 补齐「选中 → 生成采购订单」；ERP-027 补齐「批次多行合并生成」；浏览器验收待 FINAL-UI-ACCEPTANCE）** | `PurchaseQuote.cs`（比价批次/多供应商/是否选中/为客户询价）、`PurchaseQuoteController`（通用 CRUD + `GET {id}/order-prefill` + `POST {id}/to-order` + `GET batch-order-plan` + `POST batch-to-order`）、`PurchaseQuoteConversion.cs`（资格守卫 + 固定映射 + 两道重复守卫 + 服务端复核 + 批次分组/合并/跳过/单号唯一 + `PurchaseQuoteBatchLookup` 预取）、`modules.js`（行操作「生成采购订单」「预填采购订单」「批次转采购订单」+ 工具栏「批次转采购订单」，仅 `已选中` 行显示；状态选项补 `已转采购订单`）、`purchase-order-conversion.js`（带入预填 / 直接生成 / 批次计划确认 + 批次生成）、`index.html`、菜单 `purchase-quote`(`init10.sql`)、`PurchaseQuoteConversionTests`（27 用例）、`PurchaseQuoteConversionBatchTests`（22 用例）、`PurchaseQuoteConversionUiTests`（4 个 Edge 用例）；见 `docs/订单追溯字段说明.md` §4.1 / §4.3 | ① ~~`IsSelected` 选中后无「生成采购订单」下游动作~~ → **ERP-020 已实现**；② ~~比价批次多行合并生成采购订单~~ → **ERP-027 已实现**（按供应商 + 币种 + 归属客户/销售订单 + 付款条件 + 含税分组；不兼容行独立成单、不合格行显式跳过、批次内单号唯一、重复转换守卫与单行同一套）；采购价格审批、供应商回写仍未做 | low-medium |
| 6 | 销售订单（外销合同） | **verified-complete（ERP-022 补齐执行时间线；ERP-032 出货 / 财务进度与报表；待 Edge 门禁）** | `SalesOrder.cs:56-118`、`SalesOrderController`（含 `/{id}/timeline`、`/{id}/progress`、`shipment-finance-report`）、`OrderExecutionTimeline`（订单 + 出库 + 出口单证 + 客诉，固定数量查询）、`SalesOrderProgress`（已审核出库派生已订/已出/未出 + 既有引用可用时的已关联/未覆盖收款，无引用时为未知）、`SalesOrderShipmentFinanceReport`（按客户 + 币种分组的只读报表）、`order-timeline.js`、`sales-order-progress.js`、`OrderExecutionTimelineTests`、`SalesOrderShipmentFinanceReportTests`（24 用例）；另见 `OrderTraceabilityTests` 与 `docs/订单追溯字段说明.md` §4.5 | 订单变更申请、附件；~~订单执行跟踪时间轴~~ → **ERP-022 已实现**；~~销售订单出货 / 收款进度（出库与收款链接只读派生）~~ → **ERP-032 已实现**（不叠加库存流水、不冲减销售退货、无权威引用时金额为未知，不是应收余额）；SP 版单据页已退出菜单（历史 SP 表数据不迁移） | medium |
| 7 | 采购订单 | **verified-complete（ERP-022 执行时间线；ERP-026 收货 / 结算进度只读派生；ERP-031 供应商采购敞口报表；浏览器验收待 FINAL-UI-ACCEPTANCE）** | `PurchaseOrder.cs:44-86`、`PurchaseOrderController`（含 `/{id}/timeline`、`/{id}/progress`）、`OrderExecutionTimeline`（订单 + 供应商确认交期 + 入库）、`PurchaseOrderProgress`（入库派生已订/已收/未收 + 既有引用可用时的已结算/未结算，引用不唯一/不可用时金额为 null）、`order-timeline.js`、`purchase-order-progress.js`、`OrderExecutionTimelineTests`、`PurchaseOrderProgressTests`（12 用例）、`OrderTraceabilityTests`；见 `docs/订单追溯字段说明.md` §4.2 | ~~采购执行时间轴~~ → **ERP-022 已实现**；~~比价 `PurchaseQuote.IsSelected` 下游联动~~ → **ERP-020 已实现**；~~入库/结算单据进度回写（只读口径）~~ → **ERP-026 已实现派生**（不改人工字段、不写库）；仍未做：入库 / 付款单据对订单进度的自动回写、付款单按单核销明细（需结构变更任务）；~~采购入库成本单价从采购订单带入~~ → **ERP-033 已实现**（见 `docs/库存单据与库存成本说明.md` §11） | medium |
| 8 | 采购入库 / 销售出库 / 库存查询 | **verified-complete（ERP-023 基础单位换算与取消冲回；ERP-025 接入库存流水；ERP-033 采购订单成本带入）** | `Logistics.cs`、`StockInController`/`StockOutController`（审核写 `StockMovements`、取消红字冲销、`GET {id}/movements`）、`StockUnitConversion`（包装单位 → 基础单位、批量商品查询、无效因子拒绝）、`StockUnitConversionTests`、`StockInOutMovementLedgerTests`（11 用例）、`PurchaseStockInCost`（ERP-033：采购订单唯一兼容明细行单价 → 基础单位成本，包装单位按 `UnitsPerPackage` 折算、外币仅按订单持久化汇率换算、语义不明确一律回退加权平均）、`PurchaseStockInCostTests`（17 个方法 / 26 用例）、`StockController`、`modules-doc2.js:3-29` | ~~多单位换算未参与出入库计算~~ → **ERP-023 已实现**；~~历史入出库仍不写 `StockMovements`~~ → **ERP-025 已接入**（审核按基础单位写流水、取消/冲销生成红字流水、重复审核幂等拒绝、无流水历史单据取消按基础单位兜底）；剩余：~~采购入库成本单价未从采购订单带入（明细无成本列，按当前加权平均兜底）~~ → **ERP-033 已实现**（唯一兼容订单行单价 → 基础单位成本，语义不明确时仍按当前加权平均兜底，见 `docs/库存单据与库存成本说明.md` §11）、无库位/批次成本 | low-medium |
| 9 | 库存动作单据（盘点/调拨/退货）+ 库存成本 | **verified-complete（代码 + 测试；待 Edge 门禁）** | ERP-009 落地：`InventoryDocuments.cs`（`StockAdjustment`/`StockTransfer`/`SalesReturn`/`PurchaseReturn` 主子表 + `StockMovement` 流水）、`InventoryService`（移动加权平均 + 冲销）、`/api/inventory/**` 四个控制器（提交/审核/销审/取消 + `{id}/movements`）、`/api/stocks/movements`、`SchemaUpgrader.cs` 第 23 段（幂等建表/补列/菜单/字轨）、`modules-doc2.js` 5 个新页面、`InventoryMovementTests`（19 用例）、`InventoryMovementUiTests`（3 个 Edge 用例）；见 `docs/库存单据与库存成本说明.md` | 库位/批次成本、FIFO 成本法、成本调整单（库龄分层与成本估值**报表**已由 ERP-034 只读提供，见 `docs/库存单据与库存成本说明.md` §12，不改变成本口径）；~~既有采购入库/销售出库控制器未改（历史变动不入流水）~~ → **ERP-025 已接入**（见总表 #8） | medium |
...
| 13 | 单证中心 | **verified-complete（ERP-019 自动生成与 Excel 导出；ERP-030 共享打印；ERP-051 明细行表；ERP-052 生成 / 导出 / 打印消费明细行；浏览器验收待 FINAL-UI-ACCEPTANCE）** | `TradeDocument.cs`（9 类单证/4 态/关联报关单号·柜号·订单号/金额/港口/份数）、`TradeDocumentController`（+`export-excel`）、`TradeDocumentGeneration.cs`（由销售订单 / 装柜清单带入预填 + 生成 + 重复守卫 + 编号规则）、`SalesOrderController`/`ContainerLoadingListController` 各 `GET {id}/trade-documents/prefill` + `POST {id}/trade-documents`、`trade-doc-gen.js`（生成对话框 / 导出对话框 / 单条导出）、`modules.js`(`doc-center` extraActions+rowActions)、`modules-doc.js`/`modules-doc2.js`（行操作「生成单证」「预填单证」）、菜单 `doc-center`(`init11.sql`)、`TradeDocumentGenerationTests`（21 例）、`TradeDocumentGenerationUiTests`（4 例 Edge）、**ERP-030 共享打印**：`TradeDocumentPrintDtos.cs`（打印模型与字段口径）、`TradeDocumentController.GetPrint`、`PrintTemplateController`（`doc-center` 中文标题）、`trade-doc-print.js`、`TradeDocumentPrintTests`（20 例）、`docs/单证中心生成与导出说明.md` | ① ~~无「由装柜清单/销售订单自动生成」~~ → **ERP-019 已实现**（两路来源 × 带入预填/直接生成，来源留痕写入 `SalesOrderNo`/`RefNo` + 备注，重复生成守卫：同订单同类型、同柜号同类型）；② ~~无 Excel 导出~~ → **ERP-019 已实现**（列表筛选导出 + 单条导出，沿用 `ExcelExporter`）；③ ~~单证扫描件附件仍为 `FileNote` 文本~~ → **ERP-062 已交付**（单证扫描件接入同一附件证据模型：`AttachmentEvidence.OwnerType = TradeDocument`、行操作「附件证据」、工具栏「附件证据概览」按当前页 Id 批量有界计数；`FileNote` 保持原样、不解析 / 不抓取 / 不回填）；④ ~~无 PDF/版式打印（共享打印设计另立任务）~~ → **ERP-030 已实现**（打印预览 / 直接打印 / 打印设计，复用既有共享打印模板与打印设计；`GET /api/trade/documents/{id}/print` 只读投影既有字段，缺值空白、不臆造；见 `docs/单证中心生成与导出说明.md` §10）；⑤ ~~单证明细为单表，无商品明细行~~ → **ERP-051 已交付明细行表**（`TradeDocumentItems`：商业发票 / 装箱单的行级快照，服务端计算行金额、商品引用权威快照与不刷新语义、准备状态可维护 / 冻结状态只读、表头金额差异只提示）；**ERP-052 已交付生成 / 导出 / 打印消费明细行**（2026-09-25）：销售订单 / 装柜清单生成（与带入预填预览）按来源明细在**同一事务**内写入行快照（`TradeDocumentLineSnapshotRules`）、`GET /api/trade/documents/export-excel?...&layout=lines` 明细行导出（`行类型` 列 + 行金额按币种分开合计 + 箱数/重量合计 + 老单证 `无明细行` 一行）、`GET /api/trade/documents/{id}/print` 返回类型适配 `detailColumns/detailLines/detailTotals` 且 `trade-doc-print.js` 渲染明细表；单证中心新增行操作「导出明细行」与导出对话框「按明细行导出」，生成对话框展示明细行数与行快照预览；见 `docs/单证中心生成与导出说明.md` §11，测试 `src/ERP.UnitTests/TradeDocumentLineSnapshotTests.cs`（30 例）与 `TradeDocumentGenerationUiTests` 场景 5（延后执行） | low-medium |
...
| 20 | 附件中心 | partial（**ERP-061 已交付销售订单 / 采购订单、ERP-062 已交付出口单证扫描件、ERP-063 已交付验货记录与样品记录的附件内容证据、ERP-064 已交付按既有「角色 → 菜单」授权收敛的只读附件中心工作台**：唯一内容接缝 `IAttachmentContentStore` + 隔离的非生产本地存储；浏览器验收待 FINAL-UI-ACCEPTANCE） | `AttachmentEvidence`（表 `AttachmentEvidences`，SchemaUpgrader 第 40 段幂等建表）、`AttachmentEvidenceRules`/`AttachmentEvidenceService`、`AttachmentEvidenceController`（`/api/attachment-evidences`：上传 / 台账 / 详情 / 附件方式下载 / 作废 / `owner-summary` 有界计数摘要；**ERP-064** `center` / `center/summary` / `center/{id}` / `center/{id}/content` 只读工作台端点）、`IAttachmentContentStore` + `IsolatedLocalAttachmentContentStore` + `AttachmentContentStoreFactory`、`js/attachment-evidences.js` + `modules-doc.js` / `modules.js`(`doc-center` / `sample`) 行操作、「附件证据概览」与 `doc-center` 工具栏「🗂 附件中心」（`openAttachmentCenterWorkspace()`）、`docs/业务单据附件证据说明.md`、`src/ERP.UnitTests/AttachmentEvidenceTests.cs`（84 例）+ `src/ERP.UnitTests/TradeDocumentAttachmentEvidenceTests.cs`（26 例）+ `src/ERP.UnitTests/QualityInspectionSampleAttachmentEvidenceTests.cs`（40 例，ERP-063 验货记录 / 样品记录）+ `src/ERP.UnitTests/AttachmentCenterWorkspaceTests.cs`（11 例，ERP-064 授权收敛工作台） | ERP-045 仅元数据引用册仍在（职责互补：引用元数据 vs 内容证据）；生产 OSS 的凭据 / 桶配置 / 迁移与激活未实现、未注册、未激活（生产 OSS Human Gate）；`TradeDocuments.FileNote` 与 `BaseProducts.Image1~3` 保持原样、不导入不作为附件；ERP-064 工作台**只读**且只复用既有「角色 → 菜单」授权（未授权归属类型不显示记录 / 计数 / 文件名 / 摘要；无 `DataScope` 数据范围模型，如需按业务员隔离应另立任务） | low-medium |
...
| 25 | 报表中心 | verified-complete（15 张 + ERP-018/ERP-029 派生报表） | `ReportController` 16 个 `[HttpGet]`；`reports.js` 15 个 `REPORTS` 键；菜单 = 原 7 张 + `ar-aging`/`container-stats`/`purchase-cost`/`tax-refund-summary`/`stock-alert`/`sales-commission`/`follow-up-due`；**ERP-018 新增 `quotation-conversion`（报价成交率）**，入口在报价单页工具栏（报表中心菜单表未改）；**ERP-029 新增 `inventory-movement`（库存移动与呆滞报表，只读派生）**，入口在库存查询页工具栏，口径见 `docs/库存单据与库存成本说明.md` §10；**ERP-031 新增 `supplier-purchase-exposure`（供应商采购敞口报表，只读派生：按供应商 + 币种聚合采购订单金额，复用 ERP-026 / ERP-028 的权威引用口径，未链接敞口单列且不作为应付余额）**，入口在采购订单页工具栏（菜单表未改），口径见 `docs/订单追溯字段说明.md` §4.4；**ERP-032 新增 `shipment-finance-report`（销售订单出货 / 财务进度报表，只读派生：按客户 + 币种聚合「已订 / 已出 / 未出」数量与收款链接金额，复用 ERP-028 的既有引用口径，未链接 / 命中上限一律显式未知）**，入口在销售订单页工具栏与行操作（菜单表未改），口径见 `docs/订单追溯字段说明.md` §4.5；**ERP-034 新增 `inventory-aging`（库存库龄与成本估值报表，只读派生：台账 FIFO 分层 0-30 / 31-60 / 61-90 / 91-180 / 180 天以上 + 库存行持久化加权平均成本估值，库龄未知与成本未知单列、不做跨币种合并）**，入口在库存查询页工具栏（菜单表未改），口径见 `docs/库存单据与库存成本说明.md` §12 | 仍缺报表：佣金/代理费汇总、应收账龄（ERP-031 / ERP-032 是运营敞口与出货收款视图，**不是**应付 / 应收账款台账或账龄表）、~~库龄分布与成本估值~~ → **ERP-034 已实现**、生产/成本 | low-medium |
...
| 27 | 样品管理 | verified-complete（**ERP-063 已交付样品图片 / 报告作为仓库附件证据**（同一附件证据模型，`OwnerType = Sample`）；浏览器验收待 FINAL-UI-ACCEPTANCE） | `Sample`/`SampleController`、菜单 `sample`、`modules.js`(`sample` 行操作「附件证据」+ 工具栏「附件证据概览」)、`AttachmentEvidenceRules.OwnerTypeSample` | 无独立样品主数据变更；附件证据只读样品编号 / 类型 / 客户反馈原文，不作样品批准或客户确认；生产 OSS 与生产库变更仍属 Human Gate | low |
...
| 3.5 | ~~比价「选中」无下游动作~~ → **ERP-020 已解决（2026-09-24）** | `PurchaseQuote.cs:74-75` `IsSelected` + `Status` → `PurchaseQuoteController` `{id}/order-prefill` / `{id}/to-order` + `PurchaseQuoteConversion.cs` | 选中供应商后一键生成采购订单（带入预填 / 直接生成、来源留痕、重复守卫；见 §5.15） | 已关闭；比价批次多行合并生成等衍生能力按需另立任务 |
...
| 3.8 | ~~单证附件仍是文本~~ | `TradeDocument.cs:71-73` `FileNote` | ~~单证扫描件无处存放~~ → **ERP-062 已交付**：单证扫描件接入同一附件证据模型（`FileNote` 保持原样，不解析 / 不抓取 / 不回填）；**ERP-063 已交付**验货记录（既有采购订单 `QcStatus`，`OwnerType = QualityInspection`）与样品记录（既有 `Samples`，`OwnerType = Sample`）的报告 / 图片证据，均**不**新建验货 / 样品 / 存储主数据 | 附件中心（ERP-061 订单 / ERP-062 单证 / ERP-063 验货与样品；ERP-064 只读附件中心工作区） |
...
### 5.15 ERP-020 供应商比价选中行 → 采购订单（2026-09-24）

- **任务**：ERP-020「Convert selected supplier quote to Purchase Order」——关闭 §2 #5 / §3.5 / §2 #7 记录的缺口「比价 `IsSelected` 选中后无下游动作」：选中的比价行提供**带入预填**与**直接生成**两个用户可见动作，来源留痕、重复守卫、服务端复核；**不新增 / 不修改任何数据库结构**（`SchemaUpgrader.cs`、`deploy/**`、任何 `.sql`、`SeedData*.cs`、`appsettings*` 全部未改）。
- **范围与做法（关键决策）**：
  - 共享实现 `src/ERP.Api/Controllers/PurchaseQuoteConversion.cs`（静态、public，便于单测直接断言）：守卫 + 固定映射 + 两道重复守卫 + 服务端复核；控制器只做取行 / 生成单号 / 落库。
  - 来源留痕**只用既有列**（采购订单没有来源比价列）：比价行 `RefOrderNo` 写回生成的采购单号（转换前该列语义为「关联销售订单号」，转换时先解析进采购订单 `OwningSalesOrderId/No`），采购订单备注追加 `来源比价 {批次号}（比价行 #{Id}）`；比价行状态置 `已转采购订单`。
  - 重复守卫两道：① 比价行 `RefOrderNo` 指向的未删除采购订单（人工把状态改回「已选中」也不重复生成）；② 采购订单备注里的来源标记（人工清空 `RefOrderNo` 也不重复生成）；生成过的订单被软删除后允许重新生成（与 ERP-010 同一口径）。
  - `PurchaseOrderController.Calculate` / `Validate` 由 `private` 改 `public`（与 `SalesOrderController` 同款做法），保证带入路径与页面录入路径共用唯一合计 / 校验口径。
  - 「带入预填」`GET /api/purchase/quotes/{id}/order-prefill` 返回**未落库**草稿（无单号、不写库、不改来源状态），前端 `gotoModulePage('purchase-order')` + `openForm` + `fillPurchaseOrderForm` 带入后由用户核对保存；「直接生成」`POST /api/purchase/quotes/{id}/to-order` 一次落库，只 `Add` 不 `Update`。
  - 映射口径（详见 `docs/订单追溯字段说明.md` §4.1）：供应商 / 币种（枚举名不区分大小写，无法识别回退 CNY）/ 含税 / 付款条件 / 交期天数→交货日期与供应商确认交期 / 归属客户（为客户询价）/ 归属销售订单（由 `RefOrderNo` 解析）/ 明细一行（数量 × 报价单价，金额与总额服务端重算）；未列字段保持默认值（不臆造采购员、代垫、税率、执行进度）。
- **代码改动**：
  - 后端：新增 `PurchaseQuoteConversion.cs`（`BuildDraftAsync` / `FindGeneratedOrderAsync` / `MarkConverted` / `ParseCurrency` / `SourceMarker` + `PurchaseOrderPrefillResult`、`PurchaseOrderConversionResult`）；`PurchaseQuoteController` 注入 `IErpDbContext` + `IDocumentNumberService` 并新增两式；`PurchaseOrderController.Calculate/Validate` 改 public。
  - 前端：新增 `purchase-order-conversion.js`（`PURCHASE_ORDER_SOURCE`、`purchaseQuoteToOrder`、`purchaseQuotePrefillOrder`、`fillPurchaseOrderForm`）；`modules.js` 比价模块加 `rowActions`（仅 `statuses: ['已选中']` 显示）并补状态选项 `已转采购订单`；`index.html` 注册脚本。
  - 文档：`docs/订单追溯字段说明.md`（§3 接口表 + §4 页面入口 + §4.1 映射表 + §5 守卫 + §6 测试 + §7 边界）、`docs/菜单与业务流程优化建议-20260918.md`（P1-23 标记联动已实现）、本节 §5.15 与 §2 #5 / §2 #7 / §3.5 状态更新。
- **本轮实测（未启动 API、未连数据库、未运行集成/UI 用例）**：
  - safe 档：`dotnet build NEWERP.sln -c Release --no-restore /p:TreatWarningsAsErrors=true /p:RunAnalyzersDuringBuild=true` → **0 警告 / 0 错误**；`dotnet test src/ERP.UnitTests/ERP.UnitTests.csproj -c Release --no-build` → **345/345 通过**（基线 318 + `PurchaseQuoteConversionTests` 27）。
  - 门禁用例随解决方案一起编译通过（`ERP.IntegrationTests` 新增 `PurchaseQuoteConversionUiTests` 2 个 Edge 用例，`Collection=UiTests`）。
  - 前端脚本以静态断言锁定「行操作函数名 ↔ 脚本定义 ↔ 接口路径 ↔ `index.html` 注册」与报文 JSON 契约（camelCase + 枚举名），避免「按钮点了没反应」。
- **真实 Edge 验收用例（新增 `PurchaseQuoteConversionUiTests`，2 个用例）**：① 比价页「生成采购订单」→ 核对单号前缀 `PO`、总额重算（故意写错的报价总额不采信）、付款条件、明细数量/单价、备注来源标记、比价行状态 `已转采购订单` 与 `RefOrderNo` 回写，并在采购订单列表/编辑页回显带入值；② 「预填采购订单」→ 带入表单不落库、不改来源状态，随后重复调用转换接口被守卫拒绝且只存在一张采购订单。测试数据经应用自身接口创建（不直连数据库、不使用生产数据、不执行 SQL）。
- **未做的事（边界）**：未启动 `ERP.Api`、未连业务库、未执行任何 SQL/seed/部署、未运行集成/UI 用例（真实 Edge 门禁归 orchestrator，按 `defer_browser_during_development` 记为 `browser_deferred`）；未修改 `.env.local`、`deploy/**`、`release/**`、`checkpoints/**`、`logs/**`、任何 `.sql` 文件、`SchemaUpgrader.cs` 与 `SeedData*.cs`；未 commit / push。比价批次多行合并生成、采购价格审批、供应商回写、比价行「同时保留销售订单号与采购单号」所需的结构变更均为后续独立任务。本轮产物为「代码 + 测试 + 文档」，交付等级为 `code_ready`，`completed` 只由 orchestrator 判定。


### 5.16 ERP-027 比价批次多行 → 采购订单（合并 / 独立 / 跳过）（2026-09-24）

- **任务**：ERP-027「Convert selected Purchase Quote batch lines to Purchase Order」——关闭 §2 #5 记录的最后一项缺口「比价批次多行合并生成采购订单」：同一比价批次（相同 `QuoteNo`）内选中且合格的报价行可以一次转成采购订单，**兼容行合并为同一张订单、不兼容行独立成单、不合格行显式跳过**；**不新增 / 不修改任何数据库结构**（`SchemaUpgrader.cs`、`deploy/**`、任何 `.sql`、`SeedData*.cs`、`appsettings*` 全部未改）。
- **范围与做法（关键决策）**：
  - 共享实现仍在 `src/ERP.Api/Controllers/PurchaseQuoteConversion.cs`（静态、public，便于单测直接断言）。批次资格判定**逐行调用既有单行权威入口** `BuildDraftAsync`，把 `BusinessException` 转为跳过原因：因此「哪些行能转」在单行路径与批次路径**永远只有一套规则**，不存在两处守卫漂移。
  - 分组键 `BatchGroupKey` = 供应商 + 币种（枚举解析后）+ 归属客户 + 归属销售订单（`RefOrderNo` 解析结果）+ 付款条件（去空格 / 不区分大小写）+ 是否含税。**只有这些采购订单表头字段全等才合并**，否则各自独立成单——不静默改写来源值、不做多数表决 / 首行覆盖。
  - 组内明细：每行来源报价对应一行明细（商品 / 规格 / 单位 / 数量 / 单价 / 金额），金额与订单总额一律服务端重算（复用 `PurchaseOrderController.Calculate` / `Validate`，不采信比价行的报价总额）；明细保留各自交期，**订单交期取组内最晚一行**（一单承诺覆盖所有行），全部零交期时留空。
  - 来源留痕沿用 ERP-020 的既有列：订单备注 = 父备注（去重）+ 每行 `来源比价 <批次号>（比价行 #<Id>）`（父备注过长时先截断父备注、**保留来源标记**，整列 500 字符）；每行明细备注写入该行来源标记（行级可追溯）；每条已转换来源行状态置 `已转采购订单`、`RefOrderNo` 写回其所属订单的采购单号；不合格行不改状态、不改 `RefOrderNo`。
  - 重复保护：沿用两道守卫（`RefOrderNo` 链接 + 备注来源标记），批次与单行两条路径互相可见（先批次转再单行转、或先单行转再批次转，都不会重复生成）；软删除生成订单后回退来源行可重新生成（与 ERP-010 / ERP-020 同口径）。
  - 单号：新增 `EnsureUniqueOrderNoAsync`——先取单据号服务候选号，与库内既有号或**本批次已分配号**冲突时按 ERP-019 单证编号约定追加 `-2 / -3 …`（单据号规则表未配置时，服务端按「单据数量 + 毫秒」兜底，同毫秒连续取号可能同号，故必须在落库前显式保证互不相同）；**全部取号发生在任何单据落库之前**，随后批次整体一次 `SaveChanges` 提交，无半成品数据。
  - 查询有界：新增 `PurchaseQuoteBatchLookup`，批次内一次性预取①批次行 `RefOrderNo` 指向的销售订单、②该批次既有来源采购订单（按单号回写 + 备注标记前缀一次查回后内存匹配），把原本的按行 N+1 压到**固定 2 次**；落库前再 1 次取回要留痕的来源行（必须写在受跟踪实体上，批次行是按 `AsNoTracking` 读出来做守卫 / 分组的）。
  - 接口：`GET /api/purchase/quotes/batch-order-plan?quoteNo=|lineId=`（**只读**计划：分组、每组行数与重算合计、未落库草稿、跳过清单与原因；不写库、不占号、不改来源状态、不刷新审计时间）与 `POST /api/purchase/quotes/batch-to-order`（Body `{ quoteNo | lineId, lineIds? }`，`lineIds` 只限定本次要转的行且必须属于该批次）；批次内无任何合格行时转换抛 `RuleConflict`（消息含原因分组计数）且不落库、不占号。
- **代码改动**：
  - 后端：`PurchaseQuoteConversion.cs` 扩展批次能力（`ResolveBatchLinesAsync` / `BuildBatchAsync` / `MergeGroup` / `BatchGroupKey` / `EnsureUniqueOrderNoAsync` / `BuildPlan` / `SkipSummary` / `MergeBatchRemark` / `ConvertBatchAsync` + `PurchaseQuoteBatchLookup` + 批次 DTO 六个）；`BuildDraftAsync` 与 `ResolveOwningSalesOrderAsync` 增加可选 `lookup` 重载（传 null 时与单行路径行为完全一致）；`PurchaseQuoteController` 新增 `batch-order-plan` / `batch-to-order` 两式。
  - 前端：`purchase-order-conversion.js` 新增 `purchaseQuoteBatchToOrder` / `purchaseQuoteBatchToOrderByNo` / `convertPurchaseQuoteBatch` / `batchPlanText` / `batchSkipText`（**先取只读计划 → 确认框说明张数、每组行数与重算合计、跳过行 → 再落库**，成功后提示采购单号清单并刷新列表）；`modules.js` 比价模块新增行操作「批次转采购订单」（仅 `已选中` 行显示）与工具栏「📦 批次转采购订单」（按批次号，`prompt` 输入）。
  - 文档：`docs/订单追溯字段说明.md`（§3 接口表 + §4 页面入口 + 新增 §4.3 批次分组 / 合并 / 跳过 / 只读计划 / 单号与有界查询 + §5 批次校验规则 + §6 测试 + §7 边界）、本节 §5.16 与 §2 #5 状态更新。
- **本轮实测（未启动 API、未连数据库、未运行集成 / UI 用例）**：
  - safe 档：`dotnet restore NEWERP.sln` + `dotnet build NEWERP.sln -c Release --no-restore --no-incremental /p:TreatWarningsAsErrors=true /p:RunAnalyzersDuringBuild=true` → **0 警告 / 0 错误**；`dotnet test src/ERP.UnitTests/ERP.UnitTests.csproj -c Release --no-build` → **416/416 通过**（基线 394 + `PurchaseQuoteConversionBatchTests` 22）。
  - 门禁用例随解决方案一起编译通过（`ERP.IntegrationTests` 的 `PurchaseQuoteConversionUiTests` 增补 2 个批次 Edge 场景，`Collection=UiTests`）。
  - 前端脚本 `node --check`：`purchase-order-conversion.js` / `modules.js` 通过；单测另以静态断言锁定「行操作与工具栏函数名 ↔ 脚本定义 ↔ 接口路径 ↔ 先计划再确认」与两个报文的 JSON 契约（camelCase + 枚举名）。
