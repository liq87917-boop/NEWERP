...
| 7 | 采购订单 | **verified-complete（ERP-022 执行时间线；ERP-026 收货 / 结算进度只读派生；ERP-031 供应商采购敞口报表；浏览器验收待 FINAL-UI-ACCEPTANCE）** | `PurchaseOrder.cs:44-86`、`PurchaseOrderController`（含 `/{id}/timeline`、`/{id}/progress`）、`OrderExecutionTimeline`（订单 + 供应商确认交期 + 入库）、`PurchaseOrderProgress`（入库派生已订/已收/未收 + 既有引用可用时的已结算/未结算，引用不唯一/不可用时金额为 null）、`order-timeline.js`、`purchase-order-progress.js`、`OrderExecutionTimelineTests`、`PurchaseOrderProgressTests`（12 用例）、`OrderTraceabilityTests`；见 `docs/订单追溯字段说明.md` §4.2 | ~~采购执行时间轴~~ → **ERP-022 已实现**；~~比价 `PurchaseQuote.IsSelected` 下游联动~~ → **ERP-020 已实现**；~~入库/结算单据进度回写（只读口径）~~ → **ERP-026 已实现派生**（不改人工字段、不写库）；仍未做：入库 / 付款单据对订单进度的自动回写、付款单按单核销明细（需结构变更任务）、采购入库成本单价从采购订单带入 | medium |
| 8 | 采购入库 / 销售出库 / 库存查询 | **verified-complete（ERP-023 基础单位换算与取消冲回；ERP-025 接入库存流水）** | `Logistics.cs`、`StockInController`/`StockOutController`（审核写 `StockMovements`、取消红字冲销、`GET {id}/movements`）、`StockUnitConversion`（包装单位 → 基础单位、批量商品查询、无效因子拒绝）、`StockUnitConversionTests`、`StockInOutMovementLedgerTests`（11 用例）、`StockController`、`modules-doc2.js:3-29` | ~~多单位换算未参与出入库计算~~ → **ERP-023 已实现**；~~历史入出库仍不写 `StockMovements`~~ → **ERP-025 已接入**（审核按基础单位写流水、取消/冲销生成红字流水、重复审核幂等拒绝、无流水历史单据取消按基础单位兜底）；剩余：采购入库成本单价未从采购订单带入（明细无成本列，按当前加权平均兜底）、无库位/批次成本 | low-medium |
