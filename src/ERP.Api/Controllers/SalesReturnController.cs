...
/// <summary>
/// 销售退货控制器（ERP-009，EF 主子表）
/// 业务规则：
/// 1) 可关联来源销售出库单（SourceStockOutId / No）做追溯，也允许无来源直接退货；
/// 2) 审核后货物退回仓库：库存增加，入库成本优先取明细成本 → 来源出库流水成本 → 当前加权平均成本，
///    实际采用的成本会回填到明细并持久化到库存流水（估价可追溯）；
/// 3) 仅已提交可审核、仅已审核可销审；销审按流水冲销（红字流水），重复动作一律拒绝。
/// </summary>
[Route("api/inventory/sales-returns")]
public class SalesReturnController : DocumentControllerBase<SalesReturn>
{
    private readonly IDocumentNumberService _noService;
    private readonly IInventoryService _inventory;

    public SalesReturnController(IErpDbContext db, IDocumentNumberService noService, IInventoryService inventory)
        : base(db)
    {
    ...
    /// <summary>分页查询（keyword 匹配单号 / 客户 / 来源出库单号 / 退货原因）</summary>
    [HttpGet]
        ...
        [FromQuery] long? warehouseId, [FromQuery] DateTime? start, [FromQuery] DateTime? end)
    {
        ...
        if (!string.IsNullOrWhiteSpace(query.Keyword))
        {
            ...
            source = source.Where(o => o.ReturnNo.Contains(kw) || o.CustomerName.Contains(kw)
                                       || o.SourceStockOutNo.Contains(kw) || o.ReturnReason.Contains(kw));
        }

        var total = await source.CountAsync();
        var items = await source.OrderByDescending(o => o.Id)
            .Skip((query.Page - 1) * query.PageSize).Take(query.PageSize).ToListAsync();
        return Ok(ApiResponse<PagedResult<SalesReturn>>.Success(new PagedResult<SalesReturn>
        {
            Items = items,
            Total = total,
            Page = query.Page,
    ...
    public async Task<IActionResult> GetById(long id)
    {
        ...
        return Ok(ApiResponse<SalesReturn>.Success(entity));
    }
    ...
    {
        await GetOrThrowAsync(id, "销售退货单不存在");
        var movements = await _inventory.ListMovementsAsync(InventoryDocumentHelper.SalesReturnType, id);
    ...
    public async Task<IActionResult> Create([FromBody] SalesReturn entity)
    {
        ...
        entity.SourceStockOutNo = await ResolveSourceStockOutNoAsync(entity.SourceStockOutId, entity.SourceStockOutNo);
    ...
    {
        var existing = await Db.SalesReturns.Include(o => o.Details)
            .FirstOrDefaultAsync(o => o.Id == id && !o.IsDeleted)
            ?? throw BusinessException.NotFound("销售退货单不存在");
        ...
        existing.SourceStockOutId = entity.SourceStockOutId;
        existing.SourceStockOutNo = await ResolveSourceStockOutNoAsync(entity.SourceStockOutId, entity.SourceStockOutNo);
        existing.ReturnReason = entity.ReturnReason;
        existing.Remark = entity.Remark;

        Db.SalesReturnDetails.RemoveRange(existing.Details);
        ...
        await Db.SaveChangesAsync();
        return Ok(ApiResponse<object>.Success(null, "销售退货单更新成功"));
    }

    /// <summary>审核：退货入库（库存增加）并写入流水，恰好一次</summary>
    ...
    public override async Task<IActionResult> Approve(long id)
    {
        ...
        if (GetStatus(entity) != DocumentStatus.Submitted)
            throw BusinessException.RuleConflict("仅已提交的销售退货单可审核");
        if (entity.WarehouseId <= 0)
            throw BusinessException.InvalidParameter("退货入库仓库必须填写");

        var details = entity.Details.Where(d => !d.IsDeleted).OrderBy(d => d.SortNo).ToList();
        if (details.Count == 0)
            throw BusinessException.RuleConflict("销售退货单无明细，不能审核");
        if (await _inventory.CountActiveMovementsAsync(InventoryDocumentHelper.SalesReturnType, entity.Id) > 0)
            throw BusinessException.RuleConflict("该销售退货单已产生库存流水，不能重复审核");
        ...
        foreach (var d in details)
        {
            ...
            // 成本口径：明细指定成本 → 来源销售出库流水成本 → 当前加权平均成本（由服务兜底）
            var sourceCost = d.UnitCost > 0
                ? 0m
                : await _inventory.ResolveSourceCostAsync(InventoryDocumentHelper.StockOutType,
                    entity.SourceStockOutId ?? 0, d.ProductId);
            var context = await InventoryDocumentHelper.BuildContextAsync(Db,
                InventoryDocumentHelper.SalesReturnType, entity.Id, entity.ReturnNo,
                InventoryMovementType.SalesReturn, entity.WarehouseId, d.ProductId, d.ProductCode, d.ProductName,
                d.Spec, d.Unit, entity.ReturnDate,
                string.IsNullOrWhiteSpace(entity.SourceStockOutNo)
                    ? "销售退货入库"
                    : $"销售退货入库（来源出库单 {entity.SourceStockOutNo}）");

            var movement = await _inventory.IncreaseAsync(context, d.Quantity,
                d.UnitCost > 0 ? d.UnitCost : sourceCost);
            d.UnitCost = movement.UnitCost;      // 回填实际入库成本，便于对账与复核
        }
        ...
        await Db.SaveChangesAsync();
        return Ok(ApiResponse<object>.Success(null, "销售退货单已审核，退货已入库"));
    }

    /// <summary>销审：按流水冲销退货入库（重复销审被拒绝）</summary>
    [HttpPost("{id:long}/unaudit")]
    public async Task<IActionResult> Unaudit(long id)
    {
        var entity = await GetOrThrowAsync(id, "销售退货单不存在");
        if (GetStatus(entity) != DocumentStatus.Approved)
            throw BusinessException.RuleConflict("仅已审核的销售退货单可销审");

        await _inventory.ReverseAsync(InventoryDocumentHelper.SalesReturnType, entity.Id, "销售退货单销审冲销");
        SetStatus(entity, DocumentStatus.Pending);
        await Db.SaveChangesAsync();
        return Ok(ApiResponse<object>.Success(null, "已销审，退货入库已冲销"));
    }
    ...
    public override async Task<IActionResult> Cancel(long id)
    {
        var entity = await GetOrThrowAsync(id, "销售退货单不存在");
        if (GetStatus(entity) == DocumentStatus.Approved)
            throw BusinessException.RuleConflict("已审核的销售退货单不能取消，请先销审");
    ...
    /// <summary>来源销售出库单号：前端只传 Id 时按库存表回填（保证溯源字段可用）</summary>
    private async Task<string> ResolveSourceStockOutNoAsync(long? sourceStockOutId, string fallback)
    {
        if (!string.IsNullOrWhiteSpace(fallback) || sourceStockOutId is null or <= 0) return fallback;
        return await Db.StockOuts.AsNoTracking()
            .Where(o => o.Id == sourceStockOutId.Value)
            .Select(o => o.StockOutNo)
            .FirstOrDefaultAsync() ?? string.Empty;
    }

    /// <summary>行号 / 金额 / 合计统一整理（后端复核，防止前端篡改合计）</summary>
    /// <remarks>只做「计算」，不动明细主键：审核时明细是已跟踪的实体，重置主键会触发 EF 的 key 修改异常。</remarks>
    private static void Normalize(SalesReturn entity)
    {
    ...
    private static void ResetDetailIds(SalesReturn entity)
    {
