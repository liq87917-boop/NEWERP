...
Object.assign(MODULES, {
  ...
  'purchase-order': {
    ...
    extraActions: [
      ...
      /* ERP-068：供应商对账与账龄工作台入口（工具栏，始终可见；只读派生：发票含税总额 / ERP-066 有效已分配 /
         算术剩余证据 + 互斥账龄桶；未知到期日单独成组、不计算账龄；不同币种分别成行、不提供跨币种总额；
         不是总账 / 应付余额 / 法定供应商对账单 / 付款授权 / 税务申报 / 结算确认） */
      { label: '🧮 对账与账龄', title: '按供应商 + 币种对账发票含税总额证据 / ERP-066 有效已分配付款引用 / 算术剩余证据，并按显式到期日与显式 as-of 日期计算互斥账龄桶（未登记到期日的发票单独成组、不计算账龄；不同币种分别成行、绝不合并，无跨币种总额；只读派生，不是总账 / 应付余额 / 法定供应商对账单 / 付款授权 / 税务申报 / 结算确认）', onclick: 'openSupplierReconciliationAgingWorkspace()' },
    ],
    rowActions: [
