...
const MODULES = {
  customer: {
    ...
    extraActions: [
      ...
      /* ERP-072：代理服务费对账与账龄工作台入口（只读派生：对账单合计证据 / ERP-071 有效分摊 / 算术剩余证据，
         并按显式到期日与显式 as-of 日期计算互斥账龄桶；未登记到期日的对账单单独成组、不计算账龄；
         不同币种分别成行、绝不合并，无跨币种总额；不新增 / 不修改任何表与列，不写库）。 */
      { label: '🧮 服务费对账与账龄', title: '按客户 + 币种对账代理服务费对账单合计证据 / ERP-071 有效收款分摊 / 算术剩余证据，并按显式到期日与显式 as-of 日期计算互斥账龄桶（未登记到期日的对账单单独成组、不计算账龄；不同币种分别成行、绝不合并，无跨币种总额；只读派生，不是总账 / 应收余额 / 法定客户对账单 / 付款通知 / 收入确认 / 税务申报 / 结算确认）', onclick: 'openAgencyServiceFeeReconciliationWorkspace()' },
    ],
    rowActions: [
      ...
      { label: '服务费对账与账龄', icon: '🧮', title: '以本客户打开代理服务费对账与账龄工作台（只读派生：对账单合计证据 / ERP-071 有效分摊 / 算术剩余证据 + 互斥账龄桶；未登记到期日单独成组；币种绝不相加；不改写对账单、分摊行、收款单与任何来源记录）', onclick: 'openAgencyServiceFeeReconciliationWorkspace' },
    ],
  },
  supplier: {
    ...
    /* ERP-038：供应商侧只读的「供货商品」货源关系视图（有界列表）。
       ...
       ERP-068：按本供应商预筛选「对账与账龄工作台」（只读派生：发票含税总额 / ERP-066 有效已分配 /
       算术剩余证据 + 互斥账龄桶；不写任何单据，不是总账 / 应付余额 / 法定对账单 / 付款授权 / 税务申报 / 结算确认）。 */
    rowActions: [
      { label: '供货商品', icon: '🏭', title: '查看该供应商为哪些商品 / 规格供货（只读，含停用关系的历史记录；不触达采购与库存）', onclick: 'openSupplierSourcing' },
      { label: '对账与账龄', icon: '🧮', title: '以本供应商打开供应商对账与账龄工作台（按供应商 + 币种对账发票含税总额证据 / ERP-066 有效已分配付款引用 / 算术剩余证据，并按显式到期日与 as-of 日期计算互斥账龄桶；未登记到期日的发票单独成组；只读派生，不写任何单据）', onclick: 'openSupplierReconciliationAgingWorkspace' },
    ],
  },
  employee: {
  ...
  'doc-center': {
    ...
    extraActions: [
      ...
      /* ERP-062：按当前页单证一次批量查看附件证据计数（有界摘要：不逐行查库、不读取任何存储内容） */
      { label: '📎 附件证据概览', onclick: 'openAttachmentEvidenceSummaryForCurrentPage()', title: '按当前页单证一次批量查看仓库附件证据条数（有效 / 已作废）与归属可用性：只显示有界计数，不逐行查库、不访问任何存储内容；内容需在登记册里显式发起带认证的下载' },
      /* ERP-064：附件中心工作台（只读、分页、有界；只显示当前账号已获菜单授权的归属类型） */
      { label: '🗂 附件中心', onclick: 'openAttachmentCenterWorkspace()', title: '打开附件中心工作台（只读、分页、有界）：按显式字段（归属类型 / 归属 Id / 归属号码快照 / 文件名快照 / 媒体类型 / 上传人 / 登记日期区间 / 状态）筛选销售订单、采购订单、出口单证、验货记录与样品记录的仓库附件证据元数据；未获菜单授权的归属类型不显示任何记录、计数、文件名或摘要；打开元数据 / 历史或下载都会由服务端重新校验当前归属访问，父单据删除 / 不可用 / 不再授权一律 fail closed' },
    ],
    ...
    rowActions: [
      ...
      /* ERP-062：本单证的扫描件附件证据（上传 / 下载 / 作废走 ERP-061 同一附件证据模型：
         内容按不可信文件处理，摘要 / 长度 / 存储键由服务端生成，作废必填原因并保留原始元数据与历史；
         既有「附件说明 / 存放位置」保持原样，不解析、不抓取、不回填；不是报关 / 报税 / 承运人确认） */
      { label: '附件证据', icon: '📎', title: '上传或查看本单证的附件内容证据（PDF / PNG / JPEG，服务端按文件签名复核；下载以附件方式返回，作废必填原因并保留原始文件名 / 摘要 / 历史；不改写单证状态与明细行，也不把上传当作报关 / 报税 / 承运人提交或确认）', onclick: 'openAttachmentEvidencesForCurrentModule' },
