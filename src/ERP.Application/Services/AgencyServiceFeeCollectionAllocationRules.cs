...
/// <summary>
/// 客户收款单 → 代理服务费对账单 收款分摊证据的纯规则（ERP-071，无数据库依赖，便于逐条单测）：
/// 状态机（有效 → 已作废）、币种与金额精度、收款单 / 对账单的**权威资格**判定（未删除 / 未取消 / 已登记）、
/// 客户与币种兼容性、双向可用金额（收款单可分摊余额 / 对账单未分摊额）、关联状态与口径文案。
/// <para>关键口径：</para>
/// <list type="number">
/// <item>一行只按**收款单 Id + 对账单 Id 的持久化标识符**建立分摊，<strong>绝不</strong>按单号文本、金额、
/// 日期或相似度匹配 / 猜测；</item>
/// <item>不读取、不修改收款单与对账单的任何字段，只保存服务端权威写入的快照；</item>
/// <item>金额按币种精度取整后必须大于 0，且不得超过**两侧**可用金额（收款单金额 − 本维度有效分摊，
/// 对账单服务端合计 − 本维度有效分摊）；</item>
/// <item>币种与原币一致：<strong>不做汇率换算、不跨币种合并</strong>；</item>
/// <item>本维度与 ERP-053 销售订单收款引用、ERP-055 销项发票分摊是**互相独立的证据维度**，绝不相加。</item>
/// </list>
/// <para>边界：本规则只做**校验与计算**，不写库、不收款、不付款、不记账、不核销、不催收或联系客户，
/// 也不改写收款单、对账单证据、协议、客户、销售订单、装柜清单、单证、发票、库存、费用与结算记录。</para>
/// </summary>
public static class AgencyServiceFeeCollectionAllocationRules
{
    // ==================== 0. 口径常量 ====================

    /// <summary>状态：有效（形成收款分摊证据，占用收款单与对账单两侧额度）</summary>
    public const int StatusActive = 1;

    /// <summary>状态：已作废（保留原始金额 / 快照 / 作废原因与审计历史，不物理删除、不重写）</summary>
    public const int StatusVoided = 2;

    /// <summary>关联状态：收款单 / 对账单在本维度尚未被任何有效分摊行指向</summary>
    ...
    /// <summary>单条对账单最多保留的有效分摊行数（保证视图与前端有界）</summary>
    public const int MaxAllocationsPerStatement = 50;
    ...
    public const int MaxStatementCandidates = 200;

    /// <summary>系统支持的币种（与收款单 / 对账单 / 协议 / 订单币种枚举同源：CNY / USD / EUR / HKD / GBP / JPY）</summary>
    ...
    public const string RuleText =
        "分摊口径：一行只能用**收款单 Id + 对账单 Id（持久化标识符）**把一张既有、未删除且未取消的收款单的"
        + "一部分（或全部）金额显式分摊到一条**已登记**的代理服务费对账单证据上；单号 / 日期 / 状态 / 客户 / "
        + "币种快照由服务端权威写入；系统**绝不**按单号文本、金额、日期或相似度猜测对应关系；"
        + "分摊金额按币种精度取整后必须大于 0，且不得超过收款单可分摊余额与对账单未分摊额中的较小者。";

    /// <summary>双向金额口径文案</summary>
    public const string AmountRuleText =
        "金额口径：分摊金额只来自授权用户**显式提交**，服务端只做币种精度取整（0.5 进位）与上限校验；"
        + "收款单可分摊余额 = 收款单金额 − **本维度**有效分摊总额，对账单未分摊额 = 对账单服务端合计 − "
    ...
    public const string DimensionSeparationText =
        "证据维度分离：本登记册的分摊只属于「客户收款 → 代理服务费对账单」这一个证据维度；"
        + "ERP-053 的「收款单 → 销售订单」收款引用与 ERP-055 的「销项发票 → 销售订单」分摊是**另外两个独立维度**；"
        + "三个维度的金额**绝不相加**、也**不**被当作几张不同的收款单，任意一个维度的变动都不影响其它维度的数值。";

    /// <summary>唯一性口径文案</summary>
    public const string UniquenessRuleText =
        "唯一性口径：同一「对账单 + 收款单」在**未作废**分摊行内唯一（重复提交一律拒绝，不合并、不覆盖）；"
        + "已作废行保留可读但不再占用额度，因此同一组合可以重新登记一条新的有效分摊行（新旧并存可查）。";

    /// <summary>历史证据只读口径文案</summary>
    public const string HistoricalEvidenceText =
        "历史证据只读：收款单被取消或软删除、对账单被作废或软删除、客户被停用 / 删除 / 改名后，"
        + "历史分摊行仍按登记当时的收款单与对账单快照可读，只显式标注不可用；"
        + "系统不会把历史行改派到别的收款单或对账单，也不提供硬删除与静默金额替换。";

    /// <summary>模块边界文案（明确不是到账凭证 / 应收台账 / 核销 / 结算 / 记账 / 税务）</summary>
    public const string BoundaryText =
        "本登记册只是代理服务费收款的分摊证据：不是银行入账 / 到账凭证、不是应收账款台账或余额、"
        + "不是货款核销、不是客户对账单、不是收入确认、不是税务（销项）判断、不是结算确认、"
        + "也不是会计凭证或总账记账分录；登记 / 作废分摊行都不会真的收款或付款、不会移动资金、"
        + "不会把对账单标记为已收款或已结清，也不改写收款单与对账单的任何既有字段。";

    // ==================== 1. 币种、金额与文本 ====================

    /// <summary>币种规范化 + 支持范围校验（收款单币种必须来自系统币种口径，才能与对账单币种权威比对）</summary>
    public static string NormalizeCurrencyStrict(string? currency)
    {
        ...
        if (!SupportedCurrencies.Contains(value, StringComparer.Ordinal))
            throw BusinessException.InvalidParameter(
                ...
                + "（收款单币种必须与对账单币种一致，系统不做汇率换算）");
        return value;
    }

    /// <summary>收款单 / 对账单金额上限口径（按币种精度取整；用于有效行分配合计的上限比对）</summary>
    ...
    /// <summary>对账单状态文案（与 ERP-070 口径一致；未知取值照实回显）</summary>
    public static string StatementStatusText(int status) => status switch
    {
        AgencyServiceFeeStatementRules.StatusDraft => "草稿",
        AgencyServiceFeeStatementRules.StatusRecorded => "已登记",
        AgencyServiceFeeStatementRules.StatusVoided => "已作废",
        _ => $"未知（{status}）"
    };
    ...
    /// <summary>
    /// 对账单能否承接新分摊行的判定（**不抛异常**，返回原因文案供界面逐条说明）：
    /// 存在、未删除、**已登记**（草稿 / 已作废拒绝）；服务端合计必须大于 0；币种必须在系统币种口径内；
    /// 且本维度有效分摊合计尚未占满服务端合计。
    /// </summary>
    public static (bool Eligible, string Text) EvaluateStatementEligibility(
        AgencyServiceFeeStatement? statement, decimal allocatedAmount)
    {
        if (statement is null || statement.IsDeleted)
            return (false, "代理服务费对账单证据不存在或已删除，不能登记分摊（历史分摊仍可读）");

        if (statement.Status != AgencyServiceFeeStatementRules.StatusRecorded)
            return (false,
                $"对账单「{statement.StatementNo}」当前状态为 {StatementStatusText(statement.Status)}，"
                + "只有**已登记**的对账单证据可以承接收款分摊（草稿尚未构成证据、已作废证据不可再分摊）");

        var currency = CurrencyAmountRules.NormalizeCurrency(statement.Currency);
        if (!SupportedCurrencies.Contains(currency, StringComparer.Ordinal))
            return (false, $"对账单币种「{currency}」不在系统币种口径内，不能登记分摊（不做汇率换算）");

        var total = AuthoritativeAmount(statement.TotalAmount, currency);
        if (total <= 0)
            return (false, $"对账单服务端合计为 {total} {currency}，没有可分摊的对账金额");

        var remaining = total - allocatedAmount;
        if (remaining <= 0)
            return (false,
                $"对账单已被本维度有效分摊行占满（已分摊 {allocatedAmount} / 服务端合计 {total} {currency}），"
                + "不能再登记分摊");

        return (true,
            $"可分摊：对账单身份 {IdentityText(statement.StatementNo)}，本维度未分摊 {remaining} {currency}");
    }

    /// <summary>校验对账单可承接新分摊（不存在 / 已删除 → 数据不存在；其余不可用原因 → 业务规则冲突）</summary>
    public static void EnsureStatementAllocatable(
        AgencyServiceFeeStatement? statement, decimal allocatedAmount, string identity)
    {
        var (eligible, text) = EvaluateStatementEligibility(statement, allocatedAmount);
        if (eligible) return;
        if (statement is null || statement.IsDeleted) throw BusinessException.NotFound(text);
        throw BusinessException.RuleConflict($"{text}（{identity}）");
    }

    // ==================== 3.1 客户 / 币种兼容与可用性文案 ====================

    /// <summary>客户与币种兼容校验（收款单客户 / 币种必须与对账单客户 / 币种**权威一致**；不做汇率换算、不跨客户合并）</summary>
    public static void EnsureCompatible(
        FinanceReceipt receipt, string receiptCurrency, AgencyServiceFeeStatement statement)
    {
        if (receipt.CustomerId != statement.CustomerId)
            throw BusinessException.RuleConflict(
                $"收款单「{receipt.ReceiptNo}」的客户（Id={receipt.CustomerId}）与对账单「{statement.StatementNo}」"
                + $"的客户（Id={statement.CustomerId}）不一致，不能登记分摊（不做跨客户合并）");

        var statementCurrency = CurrencyAmountRules.NormalizeCurrency(statement.Currency);
        if (!string.Equals(receiptCurrency, statementCurrency, StringComparison.Ordinal))
            throw BusinessException.RuleConflict(
                $"收款单「{receipt.ReceiptNo}」的币种 {receiptCurrency} 与对账单「{statement.StatementNo}」的币种 "
                + $"{statementCurrency} 不一致，不能登记分摊（不做汇率换算、不跨币种合并）");
    }
    ...
    /// <summary>对账单可用性文案（历史证据照常可读，不可用时显式说明）</summary>
    public static string StatementAvailabilityText(AgencyServiceFeeStatement? statement)
    {
        if (statement is null || statement.IsDeleted)
            return "对账单证据已删除（历史快照仍可读，不能再分摊）";
        if (statement.Status == AgencyServiceFeeStatementRules.StatusVoided)
            return "对账单证据已作废（历史快照仍可读，不能再分摊）";
        if (statement.Status == AgencyServiceFeeStatementRules.StatusDraft)
            return "对账单证据仍为草稿（历史快照仍可读，尚不能承接分摊）";
        return "对账单证据可读（只读引用，未被本模块改写）";
    }

    /// <summary>对外身份文案（分摊行提示与台账共用；不按文本模糊匹配）</summary>
    ...
    /// <summary>分摊行对外身份文案（「对账单 ← 收款单」）</summary>
    public static string AllocationIdentityText(string? statementNo, string? receiptNo)
    {
        var statement = IdentityText(statementNo);
        var receipt = (receiptNo ?? string.Empty).Trim();
        return receipt.Length == 0 ? $"{statement} ← (未填收款单号)" : $"{statement} ← {receipt}";
    }
    ...
    /// <summary>
    /// 关联状态文案（显式展示已分摊 / 未分摊金额；未分摊部分<strong>绝不</strong>被猜测到任何收款单或对账单，
